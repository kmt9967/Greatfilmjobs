...
{
	$DADMINID = intval(scriptolution_dotcom_data($_REQUEST['ADMINID']));
	if($DADMINID > 0)
	{
		...
		if($tadmins > 1)
		{ 
			$sql="DELETE FROM administrators WHERE ADMINID='".mysqli_real_escape_string($conn->_connectionID, $DADMINID)."'";
			$conn->Execute($sql);
...
if($_POST['submitform'] == "1" || ($_REQUEST['fromid']!="" || $toid>0 || $email!="" || $username!=""))
{
	if($fromid > 0)
	{
		$addtosql = "WHERE ADMINID>='".mysqli_real_escape_string($conn->_connectionID, $fromid)."'";
	...
	if($toid > 0)
	{
		$addtosql .= "AND ADMINID<='".mysqli_real_escape_string($conn->_connectionID, $toid)."'";
