...
{
	$DID = intval(scriptolution_dotcom_data($_REQUEST['ID']));
	if($DID > 0)
	{
		$sql="DELETE FROM packs WHERE ID='".mysqli_real_escape_string($conn->_connectionID, $DID)."'";
		$conn->Execute($sql);
...
$pprice = scriptolution_dotcom_data($_REQUEST['pprice']);
$pcom = scriptolution_dotcom_data($_REQUEST['pcom']);
$add1 .= "&fromid=$fromid&toid=$toid&pprice=$pprice&pcom=$pcom";
if($_POST['submitform'] == "1" || ($_REQUEST['fromid']!="" || $toid>0 || $pprice!="" || $pcom!=""))
{
	if($fromid > 0)
	{
		$addtosql = "WHERE ID>='".mysqli_real_escape_string($conn->_connectionID, $fromid)."'";
	...
	if($toid > 0)
	{
		$addtosql .= "AND ID<='".mysqli_real_escape_string($conn->_connectionID, $toid)."'";
		Stemplate::assign('toid',$toid);
	}
	if($pprice != "")
	{
		$addtosql .= "AND pprice='".mysqli_real_escape_string($conn->_connectionID, $pprice)."'";
		Stemplate::assign('pprice',$pprice);
	}
	if($pcom != "")
	{
		$addtosql .= "AND pcom='".mysqli_real_escape_string($conn->_connectionID, $pcom)."'";
