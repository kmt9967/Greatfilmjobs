...
{
	$ARID = intval(scriptolution_dotcom_data($_REQUEST['RID']));
	if($ARID > 0)
	{
		$AUID = intval(scriptolution_dotcom_data($_REQUEST['AUID']));
		if($AUID > 0)
		{
			$money = scriptolution_dotcom_data($_REQUEST['amoney']);
			$sql="UPDATE members SET funds=funds+$money WHERE USERID='".mysqli_real_escape_string($conn->_connectionID, $AUID)."'";
			$conn->Execute($sql);
			$sql="UPDATE referrals SET status='1' WHERE RID='".mysqli_real_escape_string($conn->_connectionID, $ARID)."' AND status='0'";
			$conn->Execute($sql);
