...
{
	$DPID = scriptolution_dotcom_data($_REQUEST['DIP']);
	if($DPID > 0)
	{
		$deleteme = $DPID;
		$query = "DELETE FROM bans_ips WHERE ip='".mysqli_real_escape_string($conn->_connectionID, $deleteme)."'";
		$conn->Execute($query);
...
if($ip!="")
{
	$addtosql .= "WHERE ip like'%".mysqli_real_escape_string($conn->_connectionID, $ip)."%'";
