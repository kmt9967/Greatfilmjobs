...
{
	$DAID = intval(scriptolution_dotcom_data($_REQUEST['AID']));
	if($DAID > 0)
	{
		$sql="DELETE FROM advertisements WHERE AID='".mysqli_real_escape_string($conn->_connectionID, $DAID)."'";
		$conn->Execute($sql);
...
if($_POST['asub']=="1")
{
	$AAID = scriptolution_dotcom_data($_POST['AAID']);
	...
	$sql="UPDATE advertisements SET active='".intval($aval2)."' WHERE AID='".mysqli_real_escape_string($conn->_connectionID, $AAID)."'";
	$conn->Execute($sql);
}
//ACTIVE
...
if($_POST['submitform'] == "1" || ($_REQUEST['fromid']!="" || $toid>0 || $details!="" || $active!=""))
{
	if($fromid > 0)
	{
		$addtosql = "WHERE AID>='".mysqli_real_escape_string($conn->_connectionID, $fromid)."'";
	...
	if($toid > 0)
	{
		$addtosql .= "AND AID<='".mysqli_real_escape_string($conn->_connectionID, $toid)."'";
		Stemplate::assign('toid',$toid);
	}
	if($details != "")
	{
		$addtosql .= "AND description like'%".mysqli_real_escape_string($conn->_connectionID, $details)."%'";
