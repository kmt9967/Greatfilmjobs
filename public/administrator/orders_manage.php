...
{
	$DOID = intval(scriptolution_dotcom_data($_REQUEST['OID']));
	if($DOID > 0)
	{
		$query = "select USERID, price from orders where OID='".mysqli_real_escape_string($conn->_connectionID, $DOID)."'"; 
		$executequery=$conn->execute($query);
		$RUSERID = $executequery->fields['USERID'];
		$rprice = $executequery->fields['price'];
		issue_refund_admin($RUSERID,$DOID,$rprice);
		cancel_revenue($DOID);
		$sql="UPDATE orders SET status='7' WHERE OID='".mysqli_real_escape_string($conn->_connectionID, $DOID)."'";
		$conn->Execute($sql);
...
if($_POST['submitform'] == "1" || ($_REQUEST['fromid']!="" || $toid>0 || $gtitle!="" || $username!="" || $status!=""))
{
	if($fromid > 0)
	{
		$addtosql = "AND A.OID>='".mysqli_real_escape_string($conn->_connectionID, $fromid)."'";
	...
	if($toid > 0)
	{
		$addtosql .= "AND A.OID<='".mysqli_real_escape_string($conn->_connectionID, $toid)."'";
		Stemplate::assign('toid',$toid);
	}
	if($gtitle != "")
	{
		$addtosql .= "AND C.gtitle like'%".mysqli_real_escape_string($conn->_connectionID, $gtitle)."%'";
	...
	if($status != "")
	{
		$addtosql .= "AND A.status='".mysqli_real_escape_string($conn->_connectionID, $status)."'";
