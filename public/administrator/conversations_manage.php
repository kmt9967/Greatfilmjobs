...
if($_POST['submitform'] == "1" || ($_REQUEST['fromid']!="" || $toid>0 || $msgfrom!="" || $msgto!="" || $msg!=""))
{
	...
	if($msgfrom != "")
	{
		$addtosql .= "AND B.username like'%".mysqli_real_escape_string($conn->_connectionID, $msgfrom)."%'";
		Stemplate::assign('msgfrom',$msgfrom);
	}
	if($msgto != "")
	{
		$addtosql .= "AND C.username like'%".mysqli_real_escape_string($conn->_connectionID, $msgto)."%'";
		Stemplate::assign('msgfrom',$msgfrom);
	}
	if($msg != "")
	{
		$addtosql .= "AND A.message like'%".mysqli_real_escape_string($conn->_connectionID, $msg)."%'";
