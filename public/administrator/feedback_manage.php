...
if($_REQUEST[delete]=="1")
{
	...
	if($DRID > 0)
	{
		$sql="DELETE FROM ratings WHERE RID='".mysqli_real_escape_string($conn->_connectionID, $DRID)."'";
		$conn->Execute($sql);
...
if($_POST['submitform'] == "1" || ($_REQUEST['fromid']!="" || $toid>0 || $details!="" || $gtitle!="" || $username!=""))
{
	...
	if($details != "")
	{
		$addtosql .= "AND A.comment like'%".mysqli_real_escape_string($conn->_connectionID, $details)."%'";
