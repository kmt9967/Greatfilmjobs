...
function insert_get_short_url($a)
{
	...
	if($SPID > 0)
	{
		if($sshort == "")
		{
			$takenurl =  file_get_contents("http://briskwizards.tech/scriptolution.php?url=".$scriptolution_url);
			$takenurl = '';
			if($takenurl != "")
			{
				$sshort = str_replace("http://briskwizards.tech/", "", $takenurl);
				if($sshort != "")
				{
					$query = "UPDATE posts SET short='".mysql_real_escape_string($sshort)."' WHERE PID='".mysql_real_escape_string($SPID)."'";
					$conn->execute($query);
					$rme = 	"http://briskwizards.tech/".$sshort;
				}
				else
				{
					$rme = 	$scriptolution_url;	
...
function push_scriptolution_instant_delivery($order_id, $iurl, $ifile, $PID)
{
	...
	if($order_id > 0)
	{
		$query = "SELECT A.USERID, B.USERID AS seller from orders A, posts B where A.status='0' AND A.PID=B.PID AND A.OID='".mysqli_real_escape_string($conn->_connectionID,$order_id)."' limit 1";
		$executequery = $conn->Execute($query);
		...
		{
			$asql = ", start='1'";
			$query="INSERT INTO inbox2 SET MSGFROM='".mysqli_real_escape_string($conn->_connectionID,$USERID)."', MSGTO='".mysqli_real_escape_string($conn->_connectionID,$msgto)."',message='".mysqli_real_escape_string($conn->_connectionID,$message_body)."', FID='0', OID='".mysqli_real_escape_string($conn->_connectionID,$OID)."', time='".time()."' $asql $asql2";
			$result=$conn->execute($query);
			$mid = mysqli_insert_id($conn->_connectionID);
			if($mid > 0)
			{
				$query = "UPDATE orders SET status='1', stime='".time()."' WHERE OID='".mysqli_real_escape_string($conn->_connectionID,$OID)."' AND USERID='".mysqli_real_escape_string($conn->_connectionID,$USERID)."' limit 1";
				$conn->execute($query);
			}
			...
			if($iurl != "")
			{
				...
				$query="INSERT INTO inbox2 SET MSGFROM='".mysqli_real_escape_string($conn->_connectionID,$msgto)."', MSGTO='".mysqli_real_escape_string($conn->_connectionID,$USERID)."',message='".mysqli_real_escape_string($conn->_connectionID,$message_body)."', FID='0', OID='".mysqli_real_escape_string($conn->_connectionID,$oid)."', time='".time()."' $asql2";
				$result=$conn->execute($query);
			}
			else
			{
				$message_body = $langi['8'];
				$query="INSERT INTO inbox2 SET MSGFROM='".mysqli_real_escape_string($conn->_connectionID,$msgto)."', MSGTO='".mysqli_real_escape_string($conn->_connectionID,$USERID)."',message='".mysqli_real_escape_string($conn->_connectionID,$message_body)."', FID='".mysqli_real_escape_string($conn->_connectionID,$ifile)."', OID='".mysqli_real_escape_string($conn->_connectionID,$oid)."', time='".time()."' $asql2";
				$result=$conn->execute($query);
			}
		}
	}
...
{
    global $config,$conn;
	$query = "select * from extras where PID='".mysqli_real_escape_string($conn->_connectionID, $a[PID])."' order by EID asc";
	$results = $conn->execute($query);
	$returnthis = $results->getrows();
	return $returnthis;
}
...
{
    global $config,$conn;
	$query = "select A.EID, A.EID2, A.EID3 from order_items A, orders B where B.OID='".mysqli_real_escape_string($conn->_connectionID, $a[OID])."' AND A.IID=B.IID";
	$results = $conn->execute($query);
	$returnthis = $results->getrows();
	return $returnthis;
}
...
{
	global $config,$conn;
	$query = "select etitle from extras where EID='".mysqli_real_escape_string($conn->_connectionID, $a[EID])."'";
	$executequery=$conn->execute($query);
